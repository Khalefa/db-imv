  ...
  for (uint32_t nextProbe = 0; nextProbe < num; ++nextProbe) {
    ...
    for (auto entry = buildMatch; entry != nullptr; entry = entry->next) {
      ...
      if ((buildkey == probeKey)) {
        output_build[found] = ((void*) entry);
        output_probe[found] = (nextProbe);
        ++found;
      }
    }
  }
...
size_t probe_imv(types::Integer* probe_keys, uint32_t tuple_num, runtime::Hashmap* hash_table, void** output_build, uint32_t*output_probe) {
  ...
  vectorwise::IMVState* imv_state[VECTORSIZE + 1];

  for (int i = 0; i <= imvNum; ++i) {
    imv_state[i] = new vectorwise::IMVState();
  }
  ...
  while (true) {
    ...
    switch (imv_state[k]->stage) {
      ...
      case 0: {
        ...
        _mm512_mask_compressstoreu_epi64((output_build + found), m_match, imv_state[k]->v_bucket_addrs);
        _mm256_mask_compressstoreu_epi32((output_probe + found), m_match, _mm512_cvtepi64_epi32(imv_state[k]->v_probe_offset));
...
size_t probe_simd(types::Integer* probe_keys, uint32_t probe_num, runtime::Hashmap* hash_table, void** output_build, uint32_t*output_probe) {
  ...
  for (; nextProbe < probe_num || SIMDcon->m_valid_probe;) {
    ...
    _mm512_mask_compressstoreu_epi64((output_build + found), m_match, SIMDcon->v_bucket_addrs);
    _mm256_mask_compressstoreu_epi32((output_probe + found), m_match, _mm512_cvtepi64_epi32(SIMDcon->v_probe_offset));
  ...
  while (done < stateNum) {
    ...
    switch (amac_state[k].stage) {
      ...
      case 1: {
        ...
        auto probeKey = probe_keys[nextProbe];
        auto probeHash = (runtime::MurMurHash()(probeKey, primitives::seed));
        ...
        amac_state[k].probeKey = probeKey.value;
      ...
      case 0: {
        ...
        if (nullptr == entry) {
          ...
          if ((buildkey == amac_state[k].probeKey)) {
            output_build[found] = entry;
            output_probe[found++] = amac_state[k].tuple_id;
          }
          entry = entry->next;
          if (nullptr == entry) {
            amac_state[k].stage = 1;
